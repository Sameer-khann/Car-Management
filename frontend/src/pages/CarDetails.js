...
const CarDetails = () => {
  ...
  useEffect(() => {
    if (id) {
     const data =  dispatch(fetchCarById(id)); // Dispatch action to fetch car details
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateCar({ id, data })) // Dispatch action to update car
      .then(() => {
    ...
    {loading ? (
      ...
      <div>
        <div className="max-w-4xl mx-auto space-y-8">
          <h1 className="text-3xl font-extrabold text-center text-gray-900">{formData.title}</h1>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            {/* Car Image Section */}
            <div className="flex flex-col items-center justify-center space-y-4">
              <div className="w-full h-80 bg-gradient-to-r from-blue-300 to-blue-500 rounded-lg overflow-hidden shadow-xl">
                <img
                  src={images?.length > 0 ? images[0] : "https://via.placeholder.com/400"}
                  alt={formData.title}
                  className="w-full h-full p-5 object-contain transition-all duration-300 transform hover:scale-105"
                />
              </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl space-y-6">
              <h2 className="text-2xl font-semibold text-center text-indigo-700">Edit Car Details</h2>
